 ...
 * Jedis管理工具
 * 
 * @package com.github.boonya.redis.RedisManager
 * @date 2017年3月17日 上午9:53:37
 * @author pengjunlin
 * @comment
 * @update
 */
public class RedisManager {
	...
	private static RedisManager ru = new RedisManager();

	private RedisManager() {
		if (pool == null) {
			//Redis 安装服务器地址ifconfig查看
			String ip = "192.168.234.129";
			int port = 6379;
			...
			pool=new JedisPool(config, ip, port, 60000, "123456");
		}
	...
	public static RedisManager getRu() {
		return ru;
	}

	public static void setRu(RedisManager ru) {
		RedisManager.ru = ru;
	}
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < 100; i++) {
			redisManager.set("boonya"+i, "boonya"+i);
		}
		
		for (int i = 0; i < 100; i++) {
			System.out.println(redisManager.get("boonya"+i));
		}
